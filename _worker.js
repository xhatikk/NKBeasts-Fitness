...
async function newsResponse(request) {
  ...
  const lang = ['sq', 'de', 'en'].includes(requestedLang) ? requestedLang : 'sq';

  try {
    const response = await fetch(FEEDS[lang], {
      headers: {
        'User-Agent': 'NKBEASTS/1.0',
    ...
    const items = blocks.slice(0, 9).map((block, index) => {
      ...
      const source = cleanText(tag(block, 'source')) || sourceFromTitle(rawTitle);
      const title = rawTitle.endsWith(` - ${source}`)
        ? rawTitle.slice(0, -(source.length + 3)).trim()
        : rawTitle;
      const description = cleanText(tag(block, 'description'));
    ...
    return new Response(JSON.stringify({ items: [], error: String(error?.message || error) }), {
      status: 502,
      headers: {
        'content-type': 'application/json; charset=UTF-8',
