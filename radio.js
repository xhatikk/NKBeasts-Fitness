(() => {
  ...
  function draw(newsItems, language) {
    ...
    grid.innerHTML = newsItems.map((item, index) => {
      const fallbackItem =
        fallback[language][index % fallback[language].length];
    ...
    grid
      ...
      .forEach(button => {
        button.addEventListener("click", () => {
          openModal(Number(button.dataset.newsIndex));
        });
      });
  }
  ...
  function openModal(index) {
    ...
    const modalImage =
      document.getElementById("nkNewsModalImage");

    modalImage.src = safeUrl(
      item.image,
      fallback[language][0].image
    );

    modalImage.alt = item.title || "";

    document
      .getElementById("nkNewsModalMeta")
      .textContent = meta;

    document
      .getElementById("nkNewsModalTitle")
      .textContent = item.title || "";

    document
      .getElementById("nkNewsModalDescription")
      .textContent =
        item.description ||
        item.summary ||
        "";
  ...
  async function loadNews() {
    ...
    grid.innerHTML = `
      <div class="nk-news-message">
        ${escapeHtml(TEXT[language].loading)}
      </div>
    `;
    ...
    try {
      ...
      const response = await fetch(
        `/api/news?lang=${encodeURIComponent(language)}&t=${Date.now()}`,
        {
          cache: "no-store",
