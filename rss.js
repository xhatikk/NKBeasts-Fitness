(() => {
  ...
  async function loadNews() {
    const lang = language();
    grid.innerHTML = `<p class="loading">${escapeHtml(copy[lang].loading)}</p>`;
    refreshButton?.setAttribute('disabled', 'disabled');
    try {
      const response = await fetch(`/api/news?lang=${encodeURIComponent(lang)}&t=${Date.now()}`, { cache: 'no-store' });
